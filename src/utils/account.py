...
class Account:
    ...
    def check_username(self, username: str) -> typing.Optional[bool]:
        """
        Returns the username if and only if it is made up of 250 characters or less,
        all of which are composed by letters a-Z (lowercase and uppercase),
        numbers 0-9 and underscores. Returns a TypeError otherwise.
        """
        ...
        if len_username > MAX_CHARS_NO:
            raise MaxNumberOfCharactersReached(
                f"""\
                    ...
                    The one provided has: {len_username} characters.\
                """.rstrip()
            )
    ...
    def set_status(self, status: str) -> str:
        ...
        if new_status in STATUS_OPTS:
            ...
            raise NameError(
                f"""\
                    {status} is not a valid status. Please use one of the following\
                    status options: '{', '.join(STATUS_OPTS)}'.\
                """.rstrip()
            )
